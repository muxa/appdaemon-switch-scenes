...
@automation_fixture(
    ...
    args={
        "switch": MAIN_SWITCH,
        "scenes": [
            {"name": "First", "entities": {SCENE_LIGHT_1: "on", SCENE_LIGHT_2: "off"}},
            {"name": "Second", "entities": {SCENE_LIGHT_1: "off", SCENE_LIGHT_2: "on"}},
            {"name": "Both", "entities": {SCENE_LIGHT_1: "on", SCENE_LIGHT_2: "on"}},
            {"name": "Off", "entities": {SCENE_LIGHT_1: "off", SCENE_LIGHT_2: "off"}},
        ],
        "log_level": "DEBUG",
    },
)
...
def test_switch_starts_1s_timer_when_changed(
    hass_driver, switch_scenes_app: SwitchScenes
):
    ...
    run_in.assert_called_once_with(switch_scenes_app._on_switch_toggle_timeout, 1)


def test_switch_timout_does_not_change_scenes(
    hass_driver, switch_scenes_app: SwitchScenes
):
    with hass_driver.setup():
        hass_driver.set_state(MAIN_SWITCH, "off")
...
def test_timeout_cancelled_when_switch_toggled_within_timout(
    hass_driver, switch_scenes_app: SwitchScenes
):
    with hass_driver.setup():
        hass_driver.set_state(MAIN_SWITCH, "off")
...
def test_scene_applied_when_switch_in_off_state_toggled_within_timeout(
    hass_driver, switch_scenes_app: SwitchScenes, mocker: pytest_mock.MockerFixture
):
    with hass_driver.setup():
        hass_driver.set_state(MAIN_SWITCH, "off")
...
def test_scene_applied_when_switch_in_on_state_toggled_within_timeout(
    hass_driver, switch_scenes_app: SwitchScenes, mocker: pytest_mock.MockerFixture
):
    with hass_driver.setup():
        hass_driver.set_state(MAIN_SWITCH, "on")
...
def test_first_scene_activate_when_switch_toggled_on_last_scene(
    hass_driver, switch_scenes_app: SwitchScenes, mocker: pytest_mock.MockerFixture
):
    with hass_driver.setup():
        hass_driver.set_state(MAIN_SWITCH, "off")
...
def test_calls_service_for_all_entities_in_scene(
    hass_driver, switch_scenes_app: SwitchScenes
):
