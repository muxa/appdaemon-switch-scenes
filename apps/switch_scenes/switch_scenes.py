...
class SwitchScenes(hass.Hass):
    def initialize(self):
        ...
        self.log(
            f"Initialised Switch Scenes (switch: {self.switch}, scenes: {self.scenes})",
            level=DEBUG,
        )
    ...
    def _on_switch_state(self, entity, attribute, old, new, kwargs):
        ...
        if self.switch_tiggle_timeout_handle is None:
            self.log("Wait for toggle...", level=DEBUG)
            self.switch_tiggle_timeout_handle = self.run_in(
                self._on_switch_toggle_timeout, self.toggle_max_seconds
            )
        else:
    ...
    def apply_scene(self, scene_index):
        ...
        self.log(
            f"Activate scene {name} ({self.current_scene_index}): {entities})",
            level=DEBUG,
        )
    ...
    def _cancel_switch_toggle_timeout(self):
        if self.switch_tiggle_timeout_handle is not None:
